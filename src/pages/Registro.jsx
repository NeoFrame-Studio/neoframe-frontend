...
export function Registro() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      // Ajuste o endpoint conforme a sua API
      const response = await client.post('/auth/register', { name, email, password });
      
      // Auto-login após o registro (opcional, dependendo do seu fluxo)
      localStorage.setItem('@NeoFrame:token', response.data.token);
      window.location.href = '/dashboard'; 
      
    } catch (err) {
      setError(err.response?.data?.error || 'Erro ao criar conta. Tente novamente mais tarde.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        <div className="p-8 sm:p-10 rounded-[2.5rem] bg-white/[0.02] border border-white/[0.05] backdrop-blur-xl shadow-2xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">Junte-se à NeoFrame e comece a criar</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="flex flex-col gap-5">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-2 px-1">Nome completo</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-2 px-1">Senha</label>
              <input
                type="password"
          ...
          <div className="mt-8 text-center text-sm text-slate-500">
            Já possui uma conta? <Link to="/login" className="text-indigo-400 hover:text-indigo-300 font-medium transition-colors">Fazer login</Link>
          </div>
