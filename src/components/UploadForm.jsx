...
export default function UploadForm() {
  ...
  // =========================================================
  // NOVA LÓGICA ANTI-RACE CONDITION (V2 Portado)
  // =========================================================
  ...
  // Substituindo o startPolling(setInterval) pelo Polling Recursivo (setTimeout)
  const pollJobStatus = async (currentJobId) => {
    if (isFinalizingRef.current) return;

    try {
      const res = await client.get(`/videos/${currentJobId}`);
      const job = res.data;
      ...
      // Mantendo a lógica EXATA do seu V1 para baixar o JSON
      if (job.status === "WAITING_CURATION") {
        if (job.outputUrl) {
          ...
          if (isFinalizingRef.current) return; // Checagem extra de segurança
        ...
        return; // Retorna para parar de agendar novos pollings
      }
      ...
      // Se não caiu em nenhum if terminal, agenda a próxima checagem em 2s
  ...
  // Finalização adaptada para a V1 usando a trava do useRef
  async function finishCuration(updatedJson) {
    try {
      // 1. Trava o polling atual e limpa a memória
      ...
      // 2. Destrava e reinicia o polling para observar a renderização final
